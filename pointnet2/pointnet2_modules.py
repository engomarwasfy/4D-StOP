...
class PointnetSAModuleVotes(nn.Module):
    ...
    def __init__(
            ...
            *,
            mlp: List[int],
            mlp_binary_mask: List[int],#
            use_binary_mask: bool = None,#
            npoint: int = None,
            radius: float = None,
            nsample: int = None,
            nsample_sub: int = None,#
            bn: bool = True,
            use_xyz: bool = True,
            pooling: str = 'max',
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        if not self.ret_unique_cnt:
            ...
            grouped_features, grouped_xyz, unique_cnt, idx = self.grouper(
                xyz, new_xyz, features
            )  # (B, C, npoint, nsample), (B,3,npoint,nsample), (B,npoint)

        grouped_features_all = grouped_features#
        grouped_features = grouped_features[:, :, :, 0:self.nsample_sub]#
        ...
        if self.use_binary_mask:
            new_features_help = new_features.expand(-1,-1,-1,self.nsample)#
            grouped_features_cat = torch.cat([grouped_features_all, new_features_help], dim=1)# hier muss ich immer alle Punkte nehmen
            proposal_binary_mask = self.mlp_module_binary_mask(grouped_features_cat)#
        else:
            proposal_binary_mask = torch.zeros(1, 2, self.npoint, self.nsample).cuda()
