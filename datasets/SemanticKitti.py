...
class SemanticKittiDataset(PointCloudDataset):
    ...
    def __getitem__(self, batch_i):
        ...
        while True:
            ...
            #merged_coords = np.zeros((0, 12), dtype=np.float32)
            merged_coords = np.zeros((0, 6), dtype=np.float32)
            ...
            while num_merged < self.config.n_frames and f_ind - f_inc >= 0:
                ...
                center_labels = np.zeros((frame_points.shape[0],), dtype=np.float32)
                if self.set == 'test':
                    # Fake labels
                    sem_labels = np.zeros((frame_points.shape[0],), dtype=np.int32)
                    center_labels = np.zeros((frame_points.shape[0],1), dtype=np.float32)
                ...
                #new_points2 = hpoints.dot(pose.T)
                new_points = np.sum(np.expand_dims(hpoints, 2) * pose.T, axis=1)
                #new_points3 = new_points2.dot(np.linalg.inv(pose.T))
                ...
                # ATTENTION: In some frames nearly all points are eliminated
                ...
                if self.set in ['validation', 'test'] and self.config.n_test_frames > 1 and f_inc > 0:
                    ...
                    test_path = join(self.config.test_path, self.config.saving_path.split('/')[-1] + "_importance_None_str1_bigpug_" + str(self.config.n_test_frames) + '_' + self.config.chosen_chkp)
            ...
            #if n > self.max_in_p * self.gpu_r:                         # downsampling dependent of gpu size
            #if n > self.max_in_p:                                      # always downsampling
            if n > self.max_in_p and self.config.n_test_frames == 1:   # downsampling during training
            #if n > self.max_in_p and self.config.n_test_frames == 0:    # no downsampling
                ...
                if self.config.sampling == 'density':
                    ...
                    input_inds = np.random.choice(n, size=int(self.max_in_p), replace=False, p = (densities)/np.sum(densities))
                else:
                    #random sampling
            ...
            #in_times = in_fts[:, 11]  # hard coded last dim 
            #in_cts = in_fts[:, d_coords:11]
            in_times = in_fts[:, 5]#hard coded last dim
            #in_cts = np.zeros((in_fts.shape[0], 8))
            in_cts = np.zeros((in_fts.shape[0], 11))
            in_cts[:, 0] = in_fts[:, d_coords]
            in_fts = in_fts[:, 0:d_coords]
            ...
            if self.set in ['validation', 'test']:
                ...
                '''
                # Also apply the Data Augmentation to the object center points
                center_points = in_cts[:, 4:7].copy()
                points_and_centers = np.concatenate((in_pts, center_points), axis=0)
                augmented_points_and_centers, scale, R = self.augmentation_transform(points_and_centers)
                # ATTENTION: Same input -> different outputs (1 -> 0.9999, 1 -> 1.0001)
                augmented_points_and_centers = np.split(augmented_points_and_centers, [in_pts.shape[0]], axis=0)
                augmented_points = augmented_points_and_centers[0]
                augmented_center_points = augmented_points_and_centers[1]
                in_pts = augmented_points
                non_zero_ids = np.where((in_cts[:, 0] != 0) | (in_cts[:, 1] != 0) | (in_cts[:, 2] != 0) | (in_cts[:, 3] != 0) | (in_cts[:, 4] != 0) | (in_cts[:, 5] != 0) | (in_cts[:, 6] != 0))[0]
                in_cts[non_zero_ids, 4:7] = augmented_center_points[non_zero_ids]
                # After augmentation need to recompute the offset
                in_cts[non_zero_ids, 1:4] = in_cts[non_zero_ids, 4:7] - in_pts[non_zero_ids]
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                '''
            ...
            # After augmentation and merging, compute center and radius GT for the objects in the 4D-Volumes
            if self.set == 'training':
            #if self.set == 'training' or self.set == 'validation':
                center_gt, radius_gt, bb_size_gt = get_center_and_radius_gt(in_pts, in_slbls, in_lbls)
                in_cts[:, 1:7] = center_gt[:, 1:7]
                in_cts[:, 7] = radius_gt[:, 0]
                in_cts[:, 8:11] = bb_size_gt[:, 0:3]
            ...
            # If we want to reduce the number of 4D-Volumes in one batch to 1
            #break
            # ATTENTION: It can happen, that a 4D-Volume contains only few points (<6). Then we get a problem during Batch Normalization. Should skip these 4D-Volumes
        ...
        ins_labels_help = np.concatenate(ins_l_list, axis=0)
        ...
        input_list += [scales, rots, frame_inds, frame_centers, centers, times, ins_labels.astype(np.int64),
                       r_inds_list, r_mask_list, f_inc_r_inds_list, f_inc_r_mask_list, val_labels_list,
                       val_center_label_list, ins_labels_help]
...
class SemanticKittiCustomBatch:
    ...
    def __init__(self, input_list):
        ...
        # adding code here
        ind += 1
        self.val_center_labels = input_list[ind]
        ind += 1
        self.ins_labels_help = input_list[ind]
        # end adding
...
#added by me
def get_center_and_radius_gt(points, ins_labels, labels):
    covariance = np.diag(np.array([1, 1, 1]))
    center_point = np.zeros((1, 3))
    sem_ins_labels = np.unique(ins_labels)
    center_labels = np.zeros((points.shape[0], 7))
    radius_labels = np.zeros((points.shape[0], 1))
    bb_size_labels = np.zeros((points.shape[0], 3))
    for _, semins in enumerate(sem_ins_labels):
        ...
        # Cover the case where points that have the same instance-ids belongs to different semantic-classes
        sem_labels = labels[valid_ind]
        sem_labels = np.unique(sem_labels)
        if sem_labels.shape[0] > 1:
            print("Fehler")
        for _, sem_label in enumerate(sem_labels):
            valid_ind2 = np.argwhere((ins_labels == semins) & (labels == sem_label))[:, 0]
            if valid_ind2.shape[0] < 5:  # small groups
                continue

            # Compute bounding-box centers

            x_min = np.min(points[valid_ind2, 0])
            x_max = np.max(points[valid_ind2, 0])
            y_min = np.min(points[valid_ind2, 1])
            y_max = np.max(points[valid_ind2, 1])
            z_min = np.min(points[valid_ind2, 2])
            z_max = np.max(points[valid_ind2, 2])

            center_point[0][0] = (x_min + x_max) / 2
            center_point[0][1] = (y_min + y_max) / 2
            center_point[0][2] = (z_min + z_max) / 2

            gaussians = multivariate_normal.pdf(points[valid_ind2, 0:3], mean=center_point[0, :3], cov=covariance)
            gaussians = (gaussians - min(gaussians)) / (max(gaussians) - min(gaussians))
            center_labels[valid_ind2, 0] = gaussians  # first loc score for centerness
            center_labels[valid_ind2, 1:4] = center_point[0, :3] - points[valid_ind2, 0:3]  # last 3 for offset to center
            center_labels[valid_ind2, 4:7] = center_point[0, :3]

            # Compute bounding-sphere radius

            c, r2 = miniball.get_bounding_ball(points[valid_ind2])
            r = math.sqrt(r2)
            radius_labels[valid_ind2] = r

            # Compute bounding-box size
            x = x_max - x_min 
            y = y_max - y_min
            z = z_max - z_min
            bb_size_labels[valid_ind2, 0] = x
            bb_size_labels[valid_ind2, 1] = y
            bb_size_labels[valid_ind2, 2] = z

    return center_labels, radius_labels, bb_size_labels
#end added
